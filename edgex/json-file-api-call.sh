#!/bin/sh
#
# Send JSON file to an API through the gateway
# NOTE: Use -X PUT or -X POST as appropriate in the additional options
#
# Usage: json-file-api-call.sh <address> <jwt> <path> <json-file> ...
#
...
#
if [ $# -lt 4 ]; then
    echo "Usage: $0 <address> <jwt> <path> <json-file> [curl options...]"
    exit
fi
