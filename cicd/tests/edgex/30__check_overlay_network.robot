...
Check Ping Via Overlay Network
    ${output}=             Execute Command    kubectl get pod -o=custom-columns=NAME:.metadata.name,IP:.status.podIP
    ...
    ${output}=             Execute Command    kubectl exec -it ${pod_1} -c sync-app -- wget http://${pod_2_url}:8500 -O output    return_stdout=False    return_stderr=True
